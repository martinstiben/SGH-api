...
public class ScheduleGenerationService {
    ...
    public ScheduleHistoryDTO generate(ScheduleHistoryDTO request, String executedBy) {
        ...
        try {
            ...
            // Crear DTO de respuesta sin persistir historial
            ScheduleHistoryDTO response = new ScheduleHistoryDTO();
            response.setExecutedBy(executedBy);
            response.setExecutedAt(LocalDateTime.now());
            response.setStatus("SUCCESS");
            response.setTotalGenerated(totalGenerated);
            response.setMessage(buildSuccessMessage(totalGenerated, coursesWithoutAvailability.size()));
            ...
            response.setForce(request.isForce());
            ...
            response.setMessage(ex.getMessage() != null ? ex.getMessage() : "Error en la generación");
            response.setPeriodStart(request.getPeriodStart());
            response.setPeriodEnd(request.getPeriodEnd());
            response.setDryRun(request.isDryRun());
            response.setForce(request.isForce());
    ...
    private GenerationResult generateSchedulesForPeriod(LocalDate startDate, LocalDate endDate, String params) {
        ...
        for (courses course : coursesWithoutSchedule) {
            if (course.getTeacherSubject() == null) {
                ...
                coursesWithoutAvailability.add(CourseWithoutAvailabilityDTO.create(
                    course.getId(),
    ...
    private CourseWithoutAvailabilityDTO analyzeCourseUnavailability(courses course, teachers teacher, List<String> daysInPeriod) {
        ...
        for (String dayName : daysInPeriod) {
            Days dayEnum = getDayEnumFromString(dayName);
        ...
        if (!hasAnyAvailability) {
            return CourseWithoutAvailabilityDTO.create(
                course.getId(),
                course.getCourseName(),
                teacher.getId(),
        ...
        for (String dayName : daysInPeriod) {
            ...
            List<schedule> daySchedules = existingSchedules.stream()
                .filter(s -> dayEnum.equals(s.getDay()))
    ...
    private Days getDayEnumFromString(String dayName) {
        return switch (dayName) {
            case "Lunes" -> Days.Lunes;
            case "Martes" -> Days.Martes;
            case "Miércoles" -> Days.Miércoles;
            case "Jueves" -> Days.Jueves;
            case "Viernes" -> Days.Viernes;
    ...
    private List<schedule> generateSlotsInPeriodForCourse(courses course, teachers teacher, LocalTime start, LocalTime end,
        ...
        while (currentTime.isBefore(end)) {
            ...
            slot.setSubjectId(course.getTeacherSubject().getSubject());
            slot.setDay(day);
    ...
    private boolean hasConflict(schedule slot, Integer teacherId, String dayName) {
        ...
        return existingSchedules.stream()
            .filter(existing -> dayEnum.equals(existing.getDay())) // Solo del mismo día
            .anyMatch(existing ->
    ...
    private void notifyStudentsOfNewSchedules(List<schedule> newSchedules, String executedBy) {
        ...
        try {
            ...
            for (Integer courseId : courseIds) {
                ...
                List<User> students = userRepo.findByCourseIdWithDetails(courseId.longValue()).stream()
